...
def Convert_Pattern_to_Decimal(dataframe, values):
...
def Pattern_Value_Counts(dataframe, values):
...
def Dictionary_Pattern_Observed_Variables(dataframe, values):
...
def Observed_Variables_imputed_mean_cov(dataframe, values):
...
def Observed_Variables_Mean_Cov(dataframe, values):
...
print(Little_Test(dataframe, values)) 

print("Process time for Original and Albretcher Charac fun in seconds: " + str((time.time() - start)))
